...
const int lap_1 = 4e4;
const int lap_2 = 6e4;
const int step_1 = 5000;
const int step_2 = 500;

const long long tot_edge = 1e10;
...
void init_file() {
    ...
    int fd = open("data.csv", O_WRONLY | O_CREAT | O_APPEND, 0644);
...
void record_data(int iter, int edge_count, int &cc_size, int &cc_count) {
    cout << iter << "," << n << "," << edge_count << "," << cc_size << "," << cc_count << "\n";
}
...
void per_run(int iter){
    ...
    for(int i=1; i < lap_1; i += 1){
        ...
        if(i%step_1 == 0){
            record_data(iter, i, max_cc_size, cc_count);
        }
    }
